...
const WordpressUrlForm = (props) => {
  const helper = ({ isSubmitting, success, state }) => {
    let url = {
      name: "url",
      placeholder: "http://www.your-wordpress-site.com",
    };
    ...
    if (success) {
      ...
      submit = {
        button: { label: "Verified", icon: "check-circle", mode: "outlined" },
      };
    }

    let wordpressFormData = [{ text: url }, { ...submit }];
...
const WordpressAdvancedForm = (props) => {
    ...
    group: [
      {
        attr: "color",
        value: "blue",
        roles: [{ role: "Sample 1" }, { role: "Sample 2" }],
      },
    ],
  };
  ...
  return (
    <FormBuilder {...props} helper={helper} defaultValues={defaultValues} />
  );
};
