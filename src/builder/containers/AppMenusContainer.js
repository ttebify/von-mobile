...
class AppMenusContainer extends WordPressClass {
  ...
  addWordpressFormAction({ inputs, errors }, callback) {
    ...
    this.props
      .getApi(`${url}/wp-json/wp/v2/categories`, { per_page: 1 })
      .then((res) => {
  ...
  deleteMenuItem(index, target) {
    ...
    const newState = dotProp.delete(
      gState,
      `apps.${appIndex}.menus.${target}.${index}`
    );
  ...
  render() {
    ...
    const categories =
      this.props.categories && this.props.categories.data
        ? this.prepareCategories(this.props.categories.data)
        : [];

    const pages =
      this.props.pages && this.props.pages.data
        ? this.preparePosts(this.props.pages.data)
        : [];
