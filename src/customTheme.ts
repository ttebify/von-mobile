...
const theme = () => {
  ...
  const appIndex =  state && state.globalState && state.globalState.currentApp ? state.globalState.currentApp : 0;

  const apps = state && state.globalState && state.globalState.apps || [];
