...
export default class FormBuilderContainer extends Component {
  ...
  renderForm(key, args, state, handleChange, index = 0) {
    ...
    if (formRegister[key]) {
      ...
      if (key === "submit") {
        ...
        let action =
          key === "multiple_add" ? addChildMultiForm : deleteChildMultiForm;

        return <Comp key={k} action={action} {...args} />;
      } else if (
        (key === "multiple" || key === "section" || key === "flex") &&
        args["data"] &&
        ...
        return (
          <Comp key={k} state={state} name={name} {...rest} {...form}>
            {collection}
          </Comp>
        );
      } else {
  ...
  render() {
    ...
    const formData = this.props.helper
    ? this.props.helper({ isSubmitting, success, state })
    : this.props.data || [];
    ...
    return <FormWrapper style={style}>{form}</FormWrapper>;
  }
}
