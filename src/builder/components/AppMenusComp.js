...
const MenuList = ({ items, setEditModal, setDeleteDialog, target }) => {
  const showList = items.map((menu, index) => {
    ...
    const extra = menu.icon
      ? { left: () => <IconButton icon={menu.icon} /> }
      : {};
      ...
      <List.Item
        ...
        right={(props) => (
          ...
          >
            <IconButton
              icon="edit"
              onPress={() => setEditModal({ index, menu, visible, target })}
            />
            <IconButton
              icon="delete"
              onPress={() => setDeleteDialog({ visible, index, target })}
            />
          </View>
        )}
        {...extra}
  ...
  return <View style={{ marginLeft: 10 }}>{showList}</View>;
};
...
const EditMenuModal = ({ editModal, setEditModal, updateMenuItem }) => {
  ...
  return (
    <SimpleModal
      ...
      visible={visible}
      onDismiss={() => setEditModal({ visble: false })}
    >
      <FormBuilder
        helper={helper}
        clear={true}
